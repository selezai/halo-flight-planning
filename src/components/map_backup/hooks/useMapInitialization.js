...
import { loadOpenAipIcons } from '../utils/iconLoader';
import { addDebugControls } from '../utils/debugHelpers';
import { addLayerToggleControls } from '../utils/layerControls';
import { createOpenAipBaseStyle, applyOpenAipBaseStyle } from '../utils/openAipBaseStyle';
import { createRunwaySymbol, enhanceAirportStyling } from '../utils/airportSymbols';
import { createNavaidSymbols, enhanceNavaidStyling } from '../utils/navaidSymbols';
import { enhanceAirspaceStyling, createAirspacePatterns } from '../utils/airspaceEnhancer';
      ...
      try {
        ...
        map.once('load', async () => {
          try {
            ...
            // Set up click targets for interaction with all OpenAIP layers
            console.log('🔗 Setting up click handlers for OpenAIP layers...');
            
            openAipLayers.forEach(layer => {
              const layerId = layer.id;
              
              // Add click handler for all OpenAIP layers
              map.on('click', layerId, (e) => {
                console.log(`💆 Clicked on OpenAIP layer '${layerId}':`, e.features[0]?.properties);
                
                // Show a simple popup with feature info
                  ...
                  const popup = new maplibregl.Popup()
                    ...
                    .setHTML(`
                      <div style="font-family: monospace; font-size: 12px;">
                        ...
                        <strong>Type:</strong> ${layer.type}<br>
            ...
            // Create authentic aviation symbols (fixed for MapLibre GL compatibility)
            createRunwaySymbol(map);
            createNavaidSymbols(map);
            createAirspacePatterns(map);
            
            // NOTE: Disabled enhancement functions that override OpenAIP style with red overlays
            // enhanceAirportStyling(map);
            // enhanceNavaidStyling(map);
            // enhanceAirspaceStyling(map);
            ...
            addLayerToggleControls(map);
            
            // Add debug controls
            addDebugControls(map);
            
            // Add OpenAIP specific debug button
            addOpenAipDebugButton(map);
