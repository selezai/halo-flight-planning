...
function enhanceAeronauticalStyling(layer) {
  ...
  if (layerId.includes('airport')) {
    ...
    // Skip circle styling for airports since we now use authentic OpenAIP sprites
    if (layer.type === 'circle') {
      // Disable circle overlays - authentic OpenAIP sprites are now active
      console.log(`🚫 Skipping circle styling for ${layerId} - using authentic OpenAIP sprites instead`);
      // Don't return null - just skip the circle styling but allow the layer to continue processing
    }
    
    // Apply other styling for non-circle airport layers (symbol, line, fill)
    
    // Keep this code for reference but commented out since we use authentic sprites now
    /*
    if (layer.type === 'circle') {
      switch (airportType) {
        case 'major':
  ...
  // Navigation aids - skip circle styling since we now use authentic OpenAIP sprites
  if ((layerId.includes('navaid') || layerId.includes('vor') || layerId.includes('ndb')) && layer.type === 'circle') {
    // Disable circle overlays for navaids - authentic OpenAIP sprites are now active
    console.log(`🚫 Skipping navaid circle styling for ${layerId} - using authentic OpenAIP sprites instead`);
    // Skip circle styling but allow layer to continue processing for other types
  }

  // Waypoint styling - skip circle styling since we now use authentic OpenAIP sprites
  if ((layerId.includes('waypoint') || layerId.includes('fix')) && layer.type === 'circle') {
    // Disable circle overlays for waypoints - authentic OpenAIP sprites are now active
    console.log(`🚫 Skipping waypoint circle styling for ${layerId} - using authentic OpenAIP sprites instead`);
    // Skip circle styling but allow layer to continue processing for other types
  }

  // Obstacle styling - skip circle styling since we now use authentic OpenAIP sprites
  if (layerId.includes('obstacle') && layer.type === 'circle') {
    // Disable circle overlays for obstacles - authentic OpenAIP sprites are now active
    console.log(`🚫 Skipping obstacle circle styling for ${layerId} - using authentic OpenAIP sprites instead`);
    // Skip circle styling but allow layer to continue processing for other types
  }
...
// Helper function to fix icon-image expressions for MapLibre GL compatibility
function fixIconImageExpression(expression) {
  // Convert complex Mapbox GL expressions to simpler MapLibre GL compatible ones
  if (Array.isArray(expression) && expression.length > 0) {
    const [operator, ...args] = expression;
    
    switch (operator) {
      case 'case':
        // Simplify case expressions to step expressions
        return {
          'property': 'type',
          'type': 'categorical',
          'stops': extractStopsFromCase(expression),
          'default': 'airport-15' // fallback icon
        };
        
      case 'concat':
        // Handle concat expressions for dynamic icon names
        if (args.length >= 2 && args[0] === 'navaid_' && Array.isArray(args[1])) {
          // Convert ['concat', 'navaid_', ['get', 'type'], '-small'] to step expression
          return [
            'step',
            ['zoom'],
        ...
        return expression; // Keep as is if simple
...
// Helper to extract stops from case expressions
function extractStopsFromCase(caseExpression) {
  const stops = [];
  // Simple extraction - in practice this would need more sophisticated parsing
  // For now, return common OpenAIP icon mappings
  return [
    ['vor', 'navaid-vor-15'],
    ['vor_dme', 'navaid-vordme-15'],
    ['vortac', 'navaid-vortac-15'],
    ['ndb', 'navaid-ndb-15'],
...
function fixLayerCompatibility(layer) {
  ...
  // CRITICAL: Preserve icon-image for OpenAIP aviation layers
  if (fixedLayer.layout && fixedLayer.layout['icon-image']) {
    // Define aviation layer types that should keep their icons
    const aviationLayerTypes = [
      'airport', 'navaid', 'waypoint', 'obstacle', 'airspace',
      'vor', 'ndb', 'tacan', 'heliport', 'glider', 'reporting_point',
      'rc_airfield', 'hang_gliding', 'hotspot'
    ];
    
    const shouldKeepIcon = aviationLayerTypes.some(type => 
      layer.id.toLowerCase().includes(type) ||
      (layer['source-layer'] && layer['source-layer'].toLowerCase().includes(type))
    );
    
    if (!shouldKeepIcon) {
      // Only remove icon-image for non-aviation layers
      console.log(`🗑️ Removing icon-image for non-aviation layer: ${layer.id}`);
      delete fixedLayer.layout['icon-image'];
    } else {
      // Fix icon-image expressions for MapLibre GL compatibility
      console.log(`✅ Preserving icon-image for aviation layer: ${layer.id}`);
      if (Array.isArray(fixedLayer.layout['icon-image'])) {
        fixedLayer.layout['icon-image'] = fixIconImageExpression(fixedLayer.layout['icon-image']);
      }
    }
  }
...
export function convertOpenAipStyle(openAipStyle) {
  ...
  // HYBRID APPROACH: Use MapTiler sprites as base + fallback system for OpenAIP-specific icons
  if (convertedStyle.sprite !== undefined) {
    if (convertedStyle.sprite.includes('mapbox://')) {
      // Replace Mapbox sprite with MapTiler sprite (OpenAIP is built on MapTiler base)
      convertedStyle.sprite = 'http://localhost:3001/api/sprites/basic-v2';
      console.log('🎨 Replaced Mapbox sprite with MapTiler sprite via proxy:', convertedStyle.sprite);
    } else if (convertedStyle.sprite.includes('maptiler')) {
      // Proxy the existing MapTiler sprite URL
      convertedStyle.sprite = convertedStyle.sprite.replace('https://api.maptiler.com/maps/basic-v2/sprite', 'http://localhost:3001/api/sprites/basic-v2');
      console.log('🎨 Proxying MapTiler sprite URL:', convertedStyle.sprite);
    } else {
      console.log('🎨 Keeping existing sprite URL:', convertedStyle.sprite);
    }
  } else {
    // No sprite defined, add MapTiler sprite as base (OpenAIP layers will use fallback icons)
    convertedStyle.sprite = 'http://localhost:3001/api/sprites/basic-v2';
    console.log('🎨 Added MapTiler sprite as base, OpenAIP icons will use fallback system:', convertedStyle.sprite);
  }
  ...
  if (convertedStyle.layers) {
    ...
    convertedStyle.layers = convertedStyle.layers.filter(layer => {
      ...
      // COMPLETELY REMOVE ALL circle layers for aviation features since we use authentic OpenAIP sprites
      if (layer.type === 'circle') {
        // Define comprehensive list of aviation-related keywords
        const aviationKeywords = [
          'airport', 'navaid', 'vor', 'ndb', 'waypoint', 'fix', 'obstacle',
          'clicktarget', 'rc_airfield', 'hang_gliding', 'hotspot', 'reporting_point',
          'airspace', 'heliport', 'gliding', 'glider', 'winch', 'parachute',
          'military', 'civil', 'intl', 'apt', 'ad_', 'af_', 'light_aircraft',
          'tacan', 'dme', 'ils', 'beacon', 'windsock', 'fuel', 'customs'
        ];
        
        // Check if layer ID contains any aviation keywords (case-insensitive)
        const isAviationCircle = aviationKeywords.some(keyword => 
          layer.id.toLowerCase().includes(keyword.toLowerCase())
        );
        
        if (isAviationCircle) {
          console.log(`🚫 REMOVING aviation circle layer: ${layer.id} - OpenAIP sprites will be used instead`);
          return false; // Completely remove this layer
        }
        
        // Also remove any circle layer from OpenAIP sources as a safety measure
        if (openAipSources.includes(layer.source)) {
          console.log(`🚫 REMOVING OpenAIP circle layer: ${layer.id} - preventing overlay on authentic sprites`);
          return false;
        }
      }
